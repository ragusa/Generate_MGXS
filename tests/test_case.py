...
def test_source_kind_is_validated():
    ...
    with pytest.raises(ValueError, match="source kind"):
        Case(
            name="bad", materials=(material(),), energy_bounds_ev=(1.0, 2.0),
            source_kind="line", target_dimensions_cm=(1.0, 1.0, 1.0),
        )


def test_source_has_one_scientific_authority():
    uniform = Case(
        name="uniform", materials=(material(),), energy_bounds_ev=(1.0, 2.0, 5.0),
...
@pytest.mark.parametrize(
    ...
    [((0.5,), "one finite"), ((-0.1, 1.1), "nonnegative"), ((0.2, 0.2), "sum to one")],
)
def test_grouped_source_validation(probabilities, message):
    with pytest.raises(ValueError, match=message):
        Case(
            name="bad_grouped", materials=(material(),), energy_bounds_ev=(1.0, 2.0, 3.0),
            source_kind="grouped", source_probabilities=probabilities,
            target_dimensions_cm=(1.0, 1.0, 1.0),
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"isotopes": (("H1", 0.5),)}, "sum to one"),
        ({"isotopes": (("H1", -0.1), ("H2", 1.1))}, "nonnegative"),
    ],
)
def test_material_scientific_validation(kwargs, message):
    values = dict(logical_name="x", name="x", density_g_cm3=1.0, isotopes=(("H1", 1.0),))
...
def test_moderated_outer_dimensions_strictly_enclose_target():
    with pytest.raises(ValueError, match="strictly exceed"):
        Case(
            name="touching", materials=(material("target", "target"), material("mod", "moderator")),
            energy_bounds_ev=(1.0, 2.0), source_kind="uniform_energy",
            target_dimensions_cm=(1.0, 1.0, 1.0), outer_dimensions_cm=(2.0, 1.0, 2.0),
        )
